...
local function HookTooltip(tooltip)
    -- Initialize the state for the tooltip
    tooltipStates[tooltip] = {}

    -- Hook the SetHyperlink function to store the hyperlink in tooltipStates
    hooksecurefunc(tooltip, "SetHyperlink", function(tooltip, hyperlink)
        tooltipStates[tooltip].hyperlink = hyperlink
    end)

    -- No SetRecipeReagentItem in WoW 3.3.5a, so we remove this part
    -- Check if the function exists before attempting to hook it
    if _G.SetRecipeReagentItem then
        hooksecurefunc(tooltip, "SetRecipeReagentItem", function(tooltip, recipeId, reagentIndex)
            if C_TradeSkillUI and C_TradeSkillUI.GetRecipeReagentItemLink then
                tooltipStates[tooltip].hyperlink = C_TradeSkillUI.GetRecipeReagentItemLink(recipeId, reagentIndex)
            end
        end)
    end

    -- Hook the SetUnitAura function to store the aura in tooltipStates
    hooksecurefunc(tooltip, "SetUnitAura", function(tooltip, unit, index, filter)
        -- UnitAura in 3.3.5a returns name, rank, icon, count, debuffType, duration, expirationTime, unitCaster, isStealable, shouldConsolidate, spellId
        local _, _, _, _, _, _, _, _, _, spellId = UnitAura(unit, index, filter)
        tooltipStates[tooltip].aura = spellId
    end)

    -- Hook OnTooltipCleared to reset the state
